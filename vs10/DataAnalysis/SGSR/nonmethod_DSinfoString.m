function [S, ST] = nonmethod_DSinfoString(ds);
...
elseif isequal('bn', stimType), 
   if ds.midfreq<10e3, Fstr = paramString(ds.midfreq, 'Hz ');
   else, Fstr = paramString(deciRound(ds.midfreq/1e3,3), 'kHz ');
   end
end
...
try,  SPstr = localSpec(ds, stimType, knownTypes);
catch, SPstr = '**???**';
end
...
case 'fm',
case {'fs' 'fslog'},
   if all(ds.fmod==0), S = 'no mod';
   else, S = [paramString(ds.fmod) 'Hz mod, ' paramString(ds.modpercent) '% ' ];
   end
case 'ici',
