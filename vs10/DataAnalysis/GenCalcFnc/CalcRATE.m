...
DefParam.IndepVal    = NaN;          % Independant variable values of this dataset                                     
%-----------------------------------------------------------------
...
if isa(ArgIn, 'dataset')
    ds = ArgIn;
	Spt = spiketimes(ArgIn); %Make sure that dataset contains spiketime data ...
...
S = struct('calcfunc', mfilename, 'dsinfo', ds, 'param', Param, ...
    'curve', lowerFields(Curve));

%-------------------------local functions-------------------------
function Param = ExpandParam(ArgIn, Param)

iCond = ArgIn.Stim.Presentation.iCond;
iCond (iCond>ArgIn.Stim.Presentation.Ncond) = []; 
Param.isubseqs = iCond; 
if isempty(Param.isubseqs), error('Invalid value for property isubseqs.'); end

Param.ireps = ArgIn.Stim.Presentation.iRep;
