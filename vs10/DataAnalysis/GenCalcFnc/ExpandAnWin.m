...
if isa(ArgIn, 'dataset') && isvoid(ArgIn)
    AnWin = 0;
elseif isa(ArgIn, 'dataset') && ischar(AnWin) && any(strcmpi(AnWin, {'burstdur', 'stimdur'}))
    AnWin = [0 max(ArgIn.Stim.BurstDur)];
elseif isa(ArgIn, 'dataset') && ischar(AnWin) && strcmpi(AnWin, 'nostim')
    AnWin = [max(ArgIn.Stim.BurstDur), max(ArgIn.Stim.ISI)];
elseif isa(ArgIn, 'dataset') && ischar(AnWin) && strcmpi(AnWin, 'repdur')
    AnWin = [0, max(ArgIn.Stim.ISI)];
