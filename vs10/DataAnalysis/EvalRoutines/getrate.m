...
spt = spiketimes(ds);
if isempty(spt), error('Dataset doesn''t contain spiketimes.'); end
if ~all(ismember(NSub, 1:ds.Stim.Presentation.Ncond)), error('One of the requested subsequences wasn''t recorded or doesn''t exist.'); end 
if (AOff == -1), AOff = ds.Stim.BurstDur(1); end

spt = anwin(spt(NSub, :), [AOn, AOff]);
...
R = 1000 * sum(cellfun('length', spt), 2)' /(AOff - AOn) /ds.Stim.Presentation.Nrep;
