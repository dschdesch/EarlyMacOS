...
%EARLY DATA
 ...
 ITDbgn  = ds.Stim.ITD1;
 ITDrate = ds.Stim.startSpeed:ds.Stim.stepSpeed:ds.Stim.endSpeed;
 ITDend = ds.Stim.ITD2;
...
elseif strcmpi(Param.subseqinput, 'indepval') && all(ismember(iSubSeqs, ds.Stim.Presentation.X.PlotVal(1:ds.Stim.Presentation.Ncond)))
    iSubSeqs = find(ismember(ds.Stim.Presentation.X.PlotVal, iSubSeqs)); iSubSeqs = iSubSeqs(:);
elseif strcmpi(Param.subseqinput, 'subseq') && all(ismember(iSubSeqs, 1:ds.Stim.Presentation.Ncond))
...
if iscell(Param.cmppstanwin)
    if isvoid(Param.nitdds)
        if strcmpi(Param.pbratecor, 'yes')
            StimDur = [StimParam.stimdurmaster; StimParam.effstimdurslave; []];
        else
            StimDur = [StimParam.stimdurmaster; StimParam.stimdurslave; []];
        end
    else
        if strcmpi(Param.pbratecor, 'yes')
            StimDur = [StimParam.stimdurmaster; StimParam.effstimdurslave; Param.nitdds.Stim.BurstDur(:)];
        else
            StimDur = [StimParam.stimdurmaster; StimParam.stimdurslave; Param.nitdds.Stim.BurstDur(:)];
        end
    end
    for n = 1:2
...
if ~isvoid(Param.nitdds)
    Str = [Str; {sprintf('\\itNDfunction:\\rm %s <%s>', upper(Param.nitdds.ID.Experiment.ID.Name), ...
        upper([num2str(Param.nitdds.ID.iCell) '-' num2str(Param.nitdds.ID.iRecOfCell) '-' ds.StimType ]), ...
...
% if strcmpi(ds.fileformat, 'edf') && ~isempty(ds.StimParam.ShiftGWParam)
    ...
    ITDbgn     = ds.Stim.ITD1;
    ITDend     = ds.Stim.ITD2;
    ITDrate    = ds.Stim.ITDSpeed;
    DelaySlave = ds.Stim.OnsetDelay;
    StimDur = ds.Stim.Dur;
...
if ~strncmpi(ds.StimType, 'MOVN', 3) && ~strncmpi(ds.StimType, 'wav', 3)
    warning('Unknown dataset stimulus identifier: ''%s''.', ds.stimtype); %#ok
elseif strncmpi(ds.StimType, 'mov', 3)
    [MovNr, N, Err] = sscanf(lower(ds.ID.Experiment.ID.Type), 'mov%d', 1);
    MovNr = abs(MovNr);
...
FileName1 = ds.ID.Experiment.ID.Name;
...
switch ds.ID.Experiment.ID.Started(4:6)
    case 'Jan'
        mon = 01;
    case 'Feb'
        mon = 02;
...
RecTime = [ds.ID.Experiment.ID.Started(8:11) mon ds.ID.Experiment.ID.Started(1:2) ds.ID.Experiment.ID.Started(13:end)];
...
DSParam.datafile = upper(ds.ID.Experiment.ID.Name);
DSParam.cellnr   = ds.ID.iCell;
[dummy, DSParam.testnr, DSParam.dsid] = unraveldsID([ num2str(ds.ID.iCell) '-' num2str(ds.ID.iRecOfCell) '-' ds.StimType]);
...
DSParam.titlestr = {sprintf('\\bfMOVE ITD-curves for %s <%s>\\rm', DSParam.datafile,[ num2str(ds.ID.iCell) '-' num2str(ds.ID.iRecOfCell) '-' ds.StimType]); ...
