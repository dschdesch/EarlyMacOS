...
for n = 1:Nreq
    ...
    if ~isequal(NBin(n), 0)
        spt = spiketimes(ds);
        Data = CalcPSTH(spt(iSubSeqs(n), :), 'isubseqs', 'all', 'ireps', ...
...
Nrec = dsntd.Stim.Presentation.Ncond;
Nrep = dsntd.Stim.Presentation.Nrep;
iSubSeqs = 1:Nrec;
[ITD, idx] = sort(dsntd.Stim.Presentation.X.PlotVal(iSubSeqs));
iSubSeqs = iSubSeqs(idx);
spt2 = spiketimes(dsntd);
Spt = ApplyAnWin(spt2(iSubSeqs, :), Param.ndfncanwin);
