...
for n = 1:NCalc
    ...
    try 
        EffSPL = CalcEffSPL(ds1, 'filtercf', FilterCF, 'filterbw', FilterBW, 'channel', ds1.Rec.RecordInstr.Chan); % got here
    ...
    try 
        EffSPL = CalcEffSPL(ds2, 'filtercf', FilterCF, 'filterbw', FilterBW, 'channel', ds2.Rec.RecordInstr.Chan);
        if isequal(length(unique(EffSPL)), 1)
...
elseif (Nds == 2), %T = EVALSACXAC(ds1, SubSeqs1, ds2, SubSeqs2)    
    ...
    if ~isequal(ds1.ID.Experiment.ID.Name, ds2.ID.Experiment.ID.Name) || ~isequal(ds1.ID.iCell, ds1.ID.iCell)
...
if strcmpi(Param.subseqinput, 'subseq'), 
    ...
    if ~all(ismember(IndepVal1, ds1.Stim.Presentation.X.PlotVal))
        error(['One of the values for the independent variable of first' ...
            'dataset doesn''t exist.']);
    end
    if ~all(ismember(IndepVal2, ds2.Stim.Presentation.X.PlotVal))
        error(['One of the values for the independent variable of second' ...
            'dataset doesn''t exist.']);
    end
    for n = 1:length(IndepVal1), 
        iSubSeq1(n) = find(ds1.Stim.Presentation.X.PlotVal == IndepVal1(n)); 
        iSubSeq2(n) = find(ds2.Stim.Presentation.X.PlotVal == IndepVal2(n));
    end
end
Spt1 = spiketimes(ds1,1);
Spt1 = Spt1(iSubSeq1,:);
Spt2 = spiketimes(ds2,1);
Spt2 = Spt2(iSubSeq2,:);
[ds1, ds2] = deal((ds1), (ds2));

%% Assembling dataset information ...
N = length(IndepVal1);
Info.ds1.filename = ds1.ID.Experiment.ID.Name;
Info.ds1.icell    = ds1.ID.iCell;
Info.ds1.iseq     = ds1.ID.iDataset;
Info.ds1.seqid    = [num2str(ds1.ID.iCell) '-' num2str(ds1.ID.iRecOfCell) '-' ds1.StimType];
Info.ds1.mashed   = '';
if Param.ismashed
%TODO   Info.ds1.mashed = ['mashed#' num2str(ds1.ID.mashed)];
end
Info.ds2.iseq     = ds2.ID.iDataset;
Info.ds2.seqid    = [num2str(ds2.ID.iCell) '-' num2str(ds2.ID.iRecOfCell) '-' ds2.StimType];
Info.ds2.mashed   = '';
if Param.ismashed
...
for n = 1:N
    ...
    Info(n).ds1.indepunit = ds1.Stim.Presentation.X.FieldName;
    Info(n).ds2.isubseq  = iSubSeq2(n);
    Info(n).ds2.indepval = IndepVal2(n);
    Info(n).ds2.indepunit = ds2.Stim.Presentation.X.FieldName;
end
...
%function ...
StimParam.burstdur = round(min([ds1.Stim.BurstDur; ds2.Stim.BurstDur]));
StimParam.repdur   = round(min([ds1.Stim.ISI; ds1.Stim.ISI]));

%Repetition numbers for both datasets ...
StimParam.nrep = [ds1.Stim.Nrep, ds2.Stim.Nrep];
...
if ischar(Spt2) && strcmpi(Spt2, 'nodiag'),
    ...
    [Ypp, T, NC] = SPTCORR(anwin(Spt1, Param.anwin), 'nodiag',...
        Param.cormaxlag, Param.corbinwidth, WinDur, ''); %SAC...
    spkrate1 = NC.Rate1;
    nspike1 = NC.Nspike1;
    ...
    [Ynn, T, NC] = SPTCORR(anwin(Spt2, Param.anwin), 'nodiag',...
        Param.cormaxlag, Param.corbinwidth, WinDur, ''); %SAC...
    spkrate2 = NC.Rate2;
    nspike2 = NC.Nspike2;
    ...
    [Ypn, T, NC] = ...
        ...
        Param.cormaxlag, Param.corbinwidth, WinDur, ''); %XAC ...
