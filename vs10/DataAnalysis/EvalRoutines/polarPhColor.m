...
DefProps.gridmax = 2200;
DefProps.gridmin = 200;
DefProps.logscale = false;
...
Props.gridrange = [Props.gridmin Props.gridmax];
%remove isubs with a frequency outside of the gridrange
maxGrid = max(Props.gridrange);
minGrid = min(Props.gridrange);
idx = (Props.freqs <= maxGrid) & (Props.freqs >= minGrid);
if sum(idx) < length(Props.freqs)
	HIST = HIST(idx);
	Props.freqs = Props.freqs(idx);
	Props.periods = Props.periods(idx);
end
...
hhh=plot([-maxGrid -maxGrid maxGrid maxGrid],[-maxGrid maxGrid maxGrid -maxGrid]);
...
axesLabels = rmin:rinc:rmax;
% axesLabels = (rmin+rinc):rinc:rmax;
% axesLabels = fliplr(axesLabels);

% for i=(rmin+rinc):rinc:rmax
for n = 1:length(axesLabels);
	i = axesLabels(n);
	i = i - rmin;
	X = xunit*i;
	Y = yunit*i;
	hhh = plot(X,Y,Props.gridstyle,'color',Props.gridcolor,'linewidth',1,...
		'handlevisibility','off');
	textH(n) = text((i+rinc/3.5)*c220,(i+rinc/3.5)*s220, ...
		['  ' num2str(axesLabels(end - (n-1) ))],'verticalalignment','bottom',...
		'handlevisibility','on');
	set(textH(n), 'Color', Props.LabelColor);
end
...
X = (rmax-rmin)*cs;
Y = (rmax-rmin)*sn;
plot(X,Y,Props.gridstyle,'color',Props.gridcolor,'linewidth',1,...
	'handlevisibility','off');
...
% 	%stop if freq is smaller then rmin skip
% 	if Props.freqs(n) < rmin
% 		break;
% 	end
	...
	xSize = 1 - (Props.freqs(n)/max(Props.gridrange));
% 	xSize = rmax * xSize + rmin;
	xSize = rmax * xSize;
	...
	xx1 = xSize.*cos(theta-pi/nbin - pi/2);
	xx2 = xSize.*cos(theta+pi/nbin - pi/2);
	yy1 = xSize.*sin(theta-pi/nbin - pi/2);
	yy2 = xSize.*sin(theta+pi/nbin - pi/2);
	...
	for phi = 1:nbin,
		...
		q = patch([0 xx1(phi) xx2(phi)], [0 yy1(phi) yy2(phi)],...
			colors(round(rho(phi)+1), :), 'EdgeColor', 'none', 'FaceAlpha', 1 ...
			);
...
% 			'LineWidth', Props.linewidth );
	...
	xx1 = xSize.*cos(maxFysInRad-pi/nbin - pi/2);
	xx2 = xSize.*cos(maxFysInRad+pi/nbin - pi/2);
	yy1 = xSize.*sin(maxFysInRad-pi/nbin - pi/2);
	yy2 = xSize.*sin(maxFysInRad+pi/nbin - pi/2);
	q = patch([0 xx1 xx2], [0 yy1 yy2], 'w', 'EdgeAlpha', 0, 'FaceAlpha', 1);
	
	%plot white line at -fysical limit
	X = - maxFysInRad;
	xx1 = xSize.*cos(X-pi/nbin - pi/2);
	xx2 = xSize.*cos(X+pi/nbin - pi/2);
	yy1 = xSize.*sin(X-pi/nbin - pi/2);
	yy2 = xSize.*sin(X+pi/nbin - pi/2);
	q = patch([0 xx1 xx2], [0 yy1 yy2], 'w', 'EdgeAlpha', 0 ,'FaceAlpha', 1);
...
%draw white circle below lowest freq
minValue = max(Props.gridrange) * max(Props.freqs)/max(Props.gridrange);
xSize = 1 - (minValue/max(Props.gridrange));
xSize = rmax * xSize;
...
% xx1 = xSize.*cos(minValue-pi/nbin - pi/2);
% xx2 = xSize.*cos(minValue+pi/nbin - pi/2);
% yy1 = xSize.*sin(minValue-pi/nbin - pi/2);
% yy2 = xSize.*sin(minValue+pi/nbin - pi/2);
for phi = 1:nbin
	q = patch([0 xx1(phi) xx2(phi)], [0 yy1(phi) yy2(phi)], 'w', 'EdgeAlpha', 0 ,'FaceAlpha', 1);
end
