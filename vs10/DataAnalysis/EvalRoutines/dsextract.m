...
%        Param:             Struct with updated parameter list
%
% EXAMPLES
...
case 'repetitionrange'
    [datasetobj, Param] = extractrepetitions_(mydataset, Param,varargin{end});
end
end

%---------------------------- Local Functions ----------------------------
%FUNCT: extract repetition range from datasetobj
function [datasetobj, Param] = extractrepetitions_(mydataset, Param,acc)
...
% else
% - For others 
    datasetobj = struct2dataset(DStruct);
end
