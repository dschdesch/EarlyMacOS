...
%   Added xrange option
% 5/01/2014   Abel      Changed ~ to dummy if you want to ignore outputs
%                      (seems to be a problem in matlab 2009)
...
DS = read(dataset,DSParam.datafile,DSParam.iseq);
Thr = getThr4Cell(DS.Experiment, DSParam.cellnr, Param.ignoreuserdata, Param.thrsequencenr);
...
if (NPlots > 10)
    warning('EARLY:Info', 'Your are trying to display 10 plots or more, if you receive a Java heap space memory error, increase the Java memory: http://www.mathworks.com/support/solutions/en/data/1-18I2C/');
end
...
if ~isempty(Param.xrange)
	iSubSeqs = find(ismember(ds.Stim.Presentation.X.PlotVal, Param.xrange));
	if isempty(iSubSeqs)
...
elseif strcmpi(Param.subseqinput, 'indepval')
    iSubSeqs = find(ismember(ds.Stim.Presentation.X.PlotVal, Param.isubseqs));
...
[IndepVal, idx] = sort(ds.Stim.Presentation.X.PlotVal(iSubSeqs)); 
...
DSParam.testnr = ds.ID.iRecOfCell;
DSParam.dsid = ds.ID.iDataset
DSParam.seqid = [num2str(ds.ID.iCell) '-' num2str(ds.ID.iRecOfCell) '-' ds.Stim.StimType];
DSParam.iseq = ds.ID.iDataset;
...
DSParam.indepvar = ds.Stim.Presentation.X.FieldName;
...
% 	[dummy, dummy, FileName1] = unravelVMSPath(ds.GWParam.FileName);
% 	NoiseToken = sprintf('%s<%s>', upper(FileName1), upper(ds.GWParam.ID));
% else					%for Leuven data
	NoiseToken = sprintf('%s<%s>', upper(ds.ID.Experiment.ID.Name), upper([num2str(ds.ID.iCell) '-' num2str(ds.ID.iRecOfCell) '-' ds.StimType]));
% end
...
IndepUnit = ds.Stim.Presentation.X.ParUnit;
