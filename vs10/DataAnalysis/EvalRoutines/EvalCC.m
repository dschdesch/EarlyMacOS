...
elseif (nargin == 1) & ischar(varargin{1}) & strcmpi(varargin{1}, 'clrcache'),
    ...
    [Spt, Info, StimParam, Param] = ParseArgs(DefParam, varargin{:}); end
...
if strcmpi(Param.plot, 'yes'), 
   PlotCC(Info, StimParam, Param, CalcData);
end
...
if strcmpi(Param.isubseq, 'all'),
    SubSeqs = 1:NRec;
    [IndepVal, idx] = sort(ds.Stim.Presentation.X.PlotVal(SubSeqs));
    SubSeqs = SubSeqs(idx);
    Spt = spiketimes(ds);
    Spt = Spt(SubSeqs, :);
    ...
    if strcmpi(Param.subseqinput, 'indepval'), %Subsequences aren't sorted if user supplies values of independent variable explicitly ...
        if ~all(ismember(Param.isubseq, ds.Stim.Presentation.X.PlotVal(1:NRec)')), error('Invalid subsequences included for analysis.'); end
        IndepVal = Param.isubseq; 
        for n = 1:length(IndepVal), SubSeqs(n) = find(ds.Stim.Presentation.X.PlotVal == IndepVal(n)); end
        Spt = ds.Stim.SPL(SubSeqs, :);
    else,
...
Info.ds1.filename   = ds.ID.Experiment.ID.Name;
Info.ds1.icell      = ds.ID.iCell;
Info.ds1.iseq       = ds.ID.iDataset;
Info.ds1.seqid      = [num2str(ds.ID.iCell) '-' num2str(ds.ID.iRecOfCell) '-' ds.StimType];
Info.ds1.isubseq    = SubSeqs;
Info.ds1.indepval   = IndepVal;
Info.ds1.indepname  = ds.Stim.Presentation.X.ParName;
Info.ds1.indepshort = ds.Stim.Presentation.X.FieldName;
Info.ds1.indepunit  = ds.Stim.Presentation.X.ParUnit;
[Scale, Inc] = GetIndepScale(ds.Stim.Presentation.X.PlotVal(1:NRec));
...
StimParam.burstdur = round(min(ds.Stim.BurstDur));
StimParam.repdur   = round(min(ds.Stim.ISI));
...
SPL = GetSPL(ds); NRec = ds.Stim.Presentation.Ncond;
...
nX = 1; nY = 1.95; X = LeftMargin + (nX-1)*(AxWidth+UpperHorAxSpacing); Y = AxBgnHght + (nY-1)*(AxHeight+UpperVerAxSpacing);
...
%DIFCOR ...
nX = 2; nY = 1.95; X = LeftMargin + (nX-1)*(AxWidth+UpperHorAxSpacing); Y = AxBgnHght + (nY-1)*(AxHeight+UpperVerAxSpacing);
...
nX = 2; nY = 0.9; X = LeftMargin + (nX-1)*(AxWidth+UpperHorAxSpacing); Y = AxBgnHght + (nY-1)*(AxHeight+UpperVerAxSpacing);
