...
%changelog:
% - TF 1/12/2011:
% corrected by adding factor (Nrep-1) for the (basic) normalization of
% allR(n) (to coincidences/sec)

%% main program

% params
defaultParams.corbinwidth  = .05;
defaultParams.anwin        = [0 ds.Stim.BurstDur];
...
for n = 1:nseqs
    ...
    allR(n) = allR(n) / (NC.dur/1000 * NC.Nrep1 * (NC.Nrep1-1));   
end
...
ph = acos(ds.Stim.Presentation.X.PlotVal);                               %   Convert to phase angle
...
% Power Method
p = [0 1 1];
RPowers = lsqcurvefit(powerfunction, p, rho, R ,[],[]);
RnPowers = lsqcurvefit(powerfunction, p, rho, Rn ,[],[]);
% RPowers = lsqcurvefit(powerfunction, p, rho, R ,[0 0 0],[200000 200000 1000]);
