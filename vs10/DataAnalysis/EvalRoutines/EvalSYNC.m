...
if ~isempty(params.xrange)
	params.isubseqs = find(ismember(ds.Stim.Presentation.X.PlotVal, params.xrange));
...
if strcmpi(params.plot, 'yes')
    ...
    for cHistPlot = 1:nHistPlots
        ...
        textbox = textBoxObject(...
            sprintf('R: %f\nPh: %f\nRaySign: %f\nNSpk: %d\n IndepVal: %.2f', ...
            hist.r, hist.ph, hist.raysign, hist.nspk, ds.Stim.Presentation.X.PlotVal(cHistPlot)), ...
...
% 19/12/2012 TVP added recording side information to output struct
T.ds.stimchan = ds.Stim.DAC;
try
    UD = getuserdata(ds);
    RecSide = UD.Experiment.RecSide;
    T.ds.Chan1 = UD.Experiment.Chan1;
    T.ds.Chan2 = UD.Experiment.Chan2;

    if isempty(RecSide) && isfield(ds.Rec, 'RecordingSide')
        RecSide = ds.SessionInfo.RecordingSide;
    end;
    T.ds.recside = RecSide(1);
catch
    UD = [];
end
...
T.thr = getThr4Cell(ds.ID.Experiment, ds.ID.iCell, params.ignoreuserdata, params.thrsequencenr);
...
stimParam.repdur = sortByIdx_(ds.Stim.ISI, idx);
stimParam.burstdur = sortByIdx_(ds.Stim.BurstDur, idx);
stimParam.nrep = sortByIdx_(ds.Stim.Nrep, idx);
