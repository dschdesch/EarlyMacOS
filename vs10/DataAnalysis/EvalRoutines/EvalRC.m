...
if ~(strcmpi(ds.Stim.Slowest, 'SPL') || strcmpi(ds.Stim.Nextslow, 'SPL') || strcmpi(ds.Stim.Fastest, 'SPL'))
    error('Wrong type of dataset.'); 
end
...
exp_folder = folder(ds.ID.Experiment);
dataset_name = [num2str(ds.ID.Experiment.ID.Name) '_DS' num2str(zeropadint2str(ds.ID.iDataset,5)) '.EarlyDS']
Info.ds1.filename  = [exp_folder '\' dataset_name];
...
%Calculation parameters ...
Param.isubseqs = ds.Stim.Presentation.iCond;
...
StimParam.burstdur = min(ds.Stim.BurstDur);
StimParam.repdur   = min(ds.Stim.ISI);
StimParam.nrep     = min(ds.Stim.Presentation.Nrep);
StimParam.modfreq  = ds.Stim.Fmod;
