...
%   - added more better initial parameters for gaborfit
%  Mon Apr 30 2012  Abel   
%   - added .rate and .itd in output
...
%by abel
Template.ds1.rate = NaN;
Template.ds2.rate = NaN;
Template.ds1.itd = NaN;
Template.ds2.itd = NaN;
...
DefParam.fftxrange   = [0 500000];      %in Hz ...
...
DefParam.sync = 'auto';     %by Abel disable userdata
DefParam.recside = [];      %by Abel: allow user to overide recoding site detection
DefParam.chan1atear = [];   %by Abel: allow user to overide channel mappings (EDF) data
...
if (nargout >= 1), 
	...
	[rate1, itd1, rate2, itd2] = deal(nan);
	
	if isfield(CalcData.nitdp, 'y')
		rate1 = CalcData.nitdp.y;
		itd1 = CalcData.nitdp.x;
	end
	if isfield(CalcData.nitdn, 'y')
		rate2 = CalcData.nitdn.y;
		itd2 = CalcData.nitdn.x;
	end
	
    CalcData = structtemplate(CalcData, Template);
	
	CalcData.ds1.rate = torow(rate1);
	CalcData.ds1.itd = torow(itd1);
	CalcData.ds2.rate = torow(rate2);
	CalcData.ds2.itd = torow(itd2);
...
[DataFile, CellNr] = deal(dsP.ID.Experiment.ID.Name, dsP.ID.iCell);
        
if ~isempty(dsN),
    if ~CheckDS(dsN, '-'), warning(sprintf('Unknown stimulus type for second dataset: ''%s''.', dsN.stimtype)); end
    if ~isequal(DataFile, dsN.ID.Experiment.ID.Name) | ~isequal(CellNr, dsN.ID.iCell), warning('Datasets aren''t from same cell.'); end
...
if ~isempty(dsC),
    ...
    if ~isequal(DataFile, dsC.ID.Experiment.ID.Name) & ~isequal(CellNr, dsC.ID.iCell), warning('Datasets aren''t from same cell.'); end
end

if ~isempty(dsNRHO),
    if ~isa(dsNRHO, 'dataset') | ~strcmpi(dsNRHO.StimType, 'NRHO'), error('Expecting NRHO dataset.'); end
    if ~isequal(DataFile, dsNRHO.ID.Experiment.ID.Name) & ~isequal(CellNr, dsNRHO.ID.iCell), warning('Datasets aren''t from same cell.'); end
end    

%Assemble general information ...
Info.DataFile = DataFile;
Info.CellNr   = CellNr;
[dummy, Info.NITD.TestNrP, Info.NITD.dsIDP] = unraveldsID([num2str(dsP.ID.iCell) '-' num2str(dsP.ID.iRecOfCell) '-' dsP.StimType]);
if ~isempty(dsN), [dummy, Info.NITD.TestNrN, Info.NITD.dsIDN] = unraveldsID([num2str(dsN.ID.iCell) '-' num2str(dsN.ID.iRecOfCell) '-' dsN.StimType]);
else [Info.NITD.TestNrN, Info.NITD.dsIDN] = deal([]); end   
if ~isempty(dsC), [dummy, Info.NITD.TestNrC, Info.NITD.dsIDC] = unraveldsID([num2str(dsC.ID.iCell) '-' num2str(dsC.ID.iRecOfCell) '-' dsC.StimType]);
else [Info.NITD.TestNrC, Info.NITD.dsIDC] = deal([]); end    
if ~isempty(dsNRHO), [dummy, Info.NRHO.TestNr, Info.NRHO.dsID] = unraveldsID([num2str(dsNRHO.ID.iCell) '-' num2str(dsNRHO.ID.iRecOfCell) '-' dsNRHO.StimType]);
...
if strcmpi(ds.StimType, 'NITD')
    switch Type
    case '+', boolean = (ds.Corr == +1);
    case '-', boolean = (ds.Corr == -1);
    case '0', boolean = (ds.Corr ==  0); 
    end
elseif strcmpi(ds.ID.Experiment.ID.Name, 'edf'),
...
if isempty(dsN),
    ...
    %Time variables ...
    StimParam.repdur = dsP.Stim.ISI(1);
    if ~isequal(dsP.Stim.ISI(1), dsP.Stim.ISI(1)), warning('Repetition duration different for NTD datasets.'); end
    StimParam.burstdur = unique(dsP.Stim.BurstDur);
    if ~isequal(unique(dsP.Stim.BurstDur), unique(dsN.Stim.BurstDur)), warning('Stimulus duration is not equal for NTD datasets.'); end

    %Number of repetitions ...
    StimParam.nrep = [dsP.Stim.Presentation.Ncond, dsN.Stim.Presentation.Nrep];
    
    %Increment ...
    Inc1 = unique(diff(denan(dsP.Stim.Presentation.X.PlotVal)));
    if length(Inc1) ~= 1, Inc1 = NaN; end
    Inc2 = unique(diff(denan(dsN.Stim.Presentation.X.PlotVal)));
    if length(Inc2) ~= 1, Inc2 = NaN; end
    StimParam.inc = [Inc1, Inc2];
    
    %SPL ...
    SPLp = GetSPL(dsP); NRec = dsP.Stim.Presentation.Ncond;
    SPLp = unique(SPLp(1:NRec, :), 'rows');
    StimParam.spl1 = SPLp;
    SPLp = CombineSPLs(SPLp(1), SPLp(2));
    
    SPLn = GetSPL(dsN); NRec = dsN.Stim.Presentation.Ncond;
...
Range = [min(ds.Stim.Presentation.X.PlotVal) max(ds.Stim.Presentation.X.PlotVal)];
...
if ~CheckBinParam(ds, 'itd', Param), 
    [X, idx] = sort(-ds.Stim.Presentation.X.PlotVal(1:Nrec));
...
gaborParm.ampl = DIFF.envmax;
maxEst = DIFF.envpeak * 1000;
gaborParm.max = [ maxEst-(1000/DIFF.fft.df)*1000, maxEst,  maxEst+(1000/DIFF.fft.df)*1000 ];
gaborParm.width = DIFF.hhw * 1000;
gaborParm.freq = [DIFF.fft.df/2 DIFF.fft.df DIFF.fft.df*2];
gaborParm.freq = gaborParm.freq/1000000;
gaborParm.ph = ((DIFF.bestitd)*(DIFF.fft.df/1000))*2*pi *-1;
%Run gabor fit
[X, Y, Env, Constants] = gaborfit(DIFF.x, DIFF.y, Param.fitrange, Param.samplerate, gaborParm); 
...
%by abel, save freq and accfraction 
...
if ~isempty(CalcData.diff),
    ...
    if strncmpi(Param.gaborfit, 'n', 1),
		...
		%by abel: CalcData.gabor.secpeaks may be in e-notation, round it
		%for ismember + only draw second line if two members are found 
		idx = find(ismember(CalcData.gabor.x, round(CalcData.gabor.secpeaks*1000)));
        if ~isempty(idx), 
...
axis([MinX MaxX MinY MaxY]);
LegHdl = legend(LnHdl, {'FFT', 'LinFit'});
