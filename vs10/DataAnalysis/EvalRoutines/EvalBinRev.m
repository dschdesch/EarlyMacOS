...
for n = 1:length(DS)
    if n == 1
        Param.Datafile = DS(n).ID.Experiment.ID.Name;
        Param.seqID = [num2str(DS(n).ID.iCell) '-' num2str(DS(n).ID.iRecOfCell) '-' DS(n).StimType];
        Param.datasetID = DS(n).ID.iDataset;
    end
...
% get threshold info
try
    Y.thr = getThr4Cell(DS(1).ID.Experiment.ID.Name,DS(1).ID.iCell);
catch
end
if ~isfield(Y,'thr')
Y.thr = [];
...
wv1=DS.Stim.Waveform(1,1).Samples{1,1};
wv2=DS.Stim.Waveform(1,2).Samples{1,1};
wv = [wv1 wv2];
dt = 1e3./DS.Stim.Waveform.Fsam;
...
%     [wv, dt] = getstimsamples(DS, Param.icond(ii));
    wv = DS.Stim.Waveform(ii,1:2).Samples;
    dt = 1e3./DS.Stim.Waveform.Fsam;
    % Taken from Bram's revcorr -
    %Account for ITD in stimulus playback. This is only necessary for Madison datasets,
    %because for SGSR datasets the ITD is already taken care of in the waveforms returned
...
Yout.ds1.seqid = [num2str(DS(1).ID.iCell) '-' num2str(DS(1).ID.iRecOfCell) '-' DS(1).StimType];
...
if ~isempty(dsNP)
    ...
    Yout.dsnp.seqid = [num2str(dsNP(1).ID.iCell) '-' num2str(dsNP(1).ID.iRecOfCell) '-' dsNP(1).StimType];
...
if ~isempty(dsNN)
    ...
    Yout.dsnn.seqid = [num2str(dsNN(1).ID.iCell) '-' num2str(dsNN(1).ID.iRecOfCell) '-' dsNN(1).StimType];
...
Y.freq = 1e3*df*(0:nsam-1).'; % freq in Hz
% compute & apply hann window, compute complex fft spectrum
...
% else
    Str = strvcat(Param.Datafile,Param.seqID,['subseq: ' paramString(Param.icond.')],['SPL: ' num2str(ds.SPL)],['Stim BW: ' num2str(ds.LowFreq) '-' num2str(ds.HighFreq)]);
% end
...
if exist('rITD','var')
    if isfield(Y,'DiffRate')
        ...
        xplot(rITD, Y.DiffRate, 'o', 'color', 'g', 'markerfacecolor', 'g', 'markersize', 4, 'LineStyle', '-');
%         ,'linewidth', 1);
...
plot(1e-3*Y.freq, Y.PhaseSpec(:,1) + comp_delay ,'b');
plot(1e-3*Y.freq, Y.PhaseSpec(:,2) + comp_delay,'g');
xlim([min(Y.freq) max(Y.freq)]);
xlim([0 1])
xlabel('Frequency (kHz)')
ylabel('Phase (cycle)');
