...
long binSearch(double *data, long n, double value)
{
    ...
    //@invar Every element of data with position larger than r is greater than 
    //      or equal to the supplied value.
    //@invar Every element of data with a position smaller then l is less then
    //      the supplied value.
    //@invar l must always be greater than or equal to zero, r must always be less
    //      then n.
    //@var The distance between l and r, e.g. abs(l-r), must decrement by one after 
    //      each iteration.
    while (l <= r) {
        long m = (l+r)/2;
        //The double values or compared within relative precision of the floating 
        //point representation.
        if (fabs(data[m]-value) <= fabs(value*DBL_EPSILON)) return m;
        else if (data[m] < value) l = m + 1;
        else r = m - 1;
    }
    
    //Because l = r + 1 and the loop invariants still hold, the only element of the
    //array that can still be equal to the supplied value is the element at position
    //l. If this is not the case then we can be sure that all elements of the array
...
void recursiveSort(double* data, long l, long r, long* perm)
{
    ...
    else 
    {
        ...
        recursiveSort(data, l, m-1, perm); recursiveSort(data, m+1, r, perm);
    }
}
...
long partition(double* data, long l, long r, long pivotPos, long* perm)
{
    ...
    for (lastSmallerPos = l, checkPos = l+1; checkPos <= r; checkPos++) {
                ...
                (data[checkPos] < data[pivotPos]))
        {
            ...
            if (perm != NULL) swapLong(perm+lastSmallerPos, perm+checkPos);
        }
    }
     
    //Putting back pivot element in the right position.
    swapDouble(data+pivotPos, data+lastSmallerPos); 
    if (perm != NULL) swapLong(perm+pivotPos, perm+lastSmallerPos);
