...
if param.nsub
    nplots = ds.Stim.Presentation.Ncond;
...
for currentPlot = 1:nplots
    if mod(currentPlot-1, plotsPerPage)==0
        defaultPage(sprintf('%s: %s', param.mfileName, ds.ID.Experiment.ID.Name'));
    ...
    if param.nsub
        subSeqString = sprintf('%d %s', ds.Stim.Presentation.X.PlotVal(currentPlot), ...
            ds.Stim.Presentation.X.ParUnit);
