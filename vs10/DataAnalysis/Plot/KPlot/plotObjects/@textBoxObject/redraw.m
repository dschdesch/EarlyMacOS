...
%  Wed Sep 14 2011  Abel   
%   - Comment unsafe characters in text
%  Tue Nov 22 2011  Abel   
%   - Added option for resize() function where input textBoxObject() may be
%   a Panel() due to the redraw(Panel) function
%  Wed Nov 23 2011  Abel   
%   - Added line break support for text
...
%Get active Axes if second argument is a Panel() object
if isa(textBoxObject, 'Panel')
	myHdl = getHdl(textBoxObject);
	textBoxObject = hdl;
	hdl = myHdl;
end
...
%by Abel: Remove existing textbox if there
% if textBoxObject.hdl > 0
%DON'T deletion is done within Panel() redraw -> can't redraw without a
%Panel()
% delete(textBoxObject.hdl);
% end
...
%by Abel: comment unsafe characters
myText = regexprep(textBoxObject.text, '{', '\\{');
myText = regexprep(myText, '}', '\\}');

%by Abel: include break factor
strLength = length(myText);
if strLength > params.breakAt
	fact = floor(strLength/params.breakAt);
	myCellStr = cell(1, fact, []);
	myCellStr{1} = myText(1:params.breakAt);
	for n=2:fact
		myCellStr{end+1} = myText( (n-1)*params.breakAt +1 : (n)*params.breakAt );
	end
	if rem(strLength, params.breakAt) > 0
		myCellStr{end+1} = myText( fact*params.breakAt +1 : end );
	end
	myText = myCellStr;
end

plotString = 'textBoxObject.hdl = text(''String'', myText, ''units'', ''normalized''';
