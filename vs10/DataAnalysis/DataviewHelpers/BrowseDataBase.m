...
%% ---------------- CHANGELOG -----------------------
%[Mon July 2016 (Abel)]: Use filesep to be OS independent
...
switch keyword
    case 'init' % initialize BrowseData from log file
        ...
        if ~isempty(varargin{1})           
            ...
            error('Can not open Experiment:" %s". Please select another.', varargin{1});
        end
    ...
    case 'save' % save database to file, if any
        if ~isempty(BrowseData) && ~isempty(BrowseData.Experiment);
            FN = FullFileName(BrowseData.ExpID, databrowseDir, 'databrowse');
    ...
    case 'getset'
        ...
        DS = dataset([bdataDir filesep BrowseData.Experiment], iseq);
