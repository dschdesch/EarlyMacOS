...
RAT.iSub = iSub(idum);
SPT = anwin(spiketimes(ds), TW, params.iRep);  % select spikes within TW
...
ds = RAT.DSinfo;
xlabel(ds.Stim.Presentation.X.FieldName); ylabel(YLS);
...
titstr = [ds.Stim.Presentation.X.FieldName ' --- ' [ds.Stim.Presentation.Ncond ds.Stim.Presentation.Nrep ds.Stim.ISI] , ' --- burst: ' num2sstr(bd) ' ms'];
Th = title(titstr); 
