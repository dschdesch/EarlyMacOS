...
%% ---------------- CHANGELOG -----------------------
%[Mon Apr 13 2015 (Abel)]: Use filesep to be OS independent
...
case 'init',
   ...
   uiwait(hh.Root);
case 'close',
   try %by abel: start catch block here so we can still close the figure if something went wrong and we lost our figure handles
   ...
   %try
       ...
       warning('Lost my figure handles for some reason, deleting active figure instead (!check this)');
       delete(gcf) %if something went wrong and we've lost the handles 
   end
case 'update',
...
case 'collectlogs',
   ...
   qq = dir([bdataDir filesep '*.log']); %by Abel: use filesep to be OS independent
   if isempty(qq), 
