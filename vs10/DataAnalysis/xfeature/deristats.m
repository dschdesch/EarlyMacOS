function [Chunk, Raw, AP, EPSP]=deristats(D, tau, ThrAPdownRate, ThrEPSP);
% deristats - view statistics of time-derivative of recordings
%   [Chunk, Raw, AP, EPSP]=deristats(D, tau, ThrAPdownRate, ThrEPSP, ThrEPSPrate)
%   INPUTS:
...
if nargin<2, tau = 0.05; end % ms default windiw width
if nargin<3, ThrAPdownRate = -10; end % V/s default threshold neg rate for distinguishing APs
if nargin<4, ThrEPSP=[]; end % default: Median + 4 IQRs (realized below)
...
BaseLineMarginFactor = 2; % everything within 2 IQRs from Median is considered "baseline"
IqrFactor = [3 2]*BaseLineMarginFactor; % chunk criterium factors passed to Xchunks.
...
CacheParam = {D.ExpID D.RecID D.icond tau, ThrAPdownRate, ThrEPSP ...
    tau_chunk BaseLineMarginFactor IqrFactor minInflexDropRate};
...
Raw = CollectInStruct(ExpID, RecID, icond, '-', ...
    dt, getsam, getdsam, '-', ...
    tau, tau_chunk, ThrAPdownRate, ThrEPSP, '-', ...
    BaseLineMarginFactor, IqrFactor, minInflexDropRate, '-', ...
...
if isempty(iAP),
    AP = emptystruct('istart','iend','t0','t1','Vpeak','tpeak',...
        'maxDownSlope','tmaxDownSlope','Vdip','tdip','maxPostUpslope',...
        'tmaxPostUpslope','tInflex','monoUpslope','maxPreUpslope',...
        'tmaxPreUpslope','tEPSPmaxRate','EPSPmaxRate','tpeakEPSP',...
        'VpeakEPSP','itvAP','itvEvent');
...
%         'VpeakEPSP',NaN,'itvAP',NaN,'itvEvent',NaN);
    ...
    highThr = MedSam+BaseLineMarginFactor*IqrSam; % high baseline
...
if isempty(iAP), % avoid referring to APs that are not there
    iup = iprepeak; idown = iup+1;
else
    % find those peaks that are *completely* swallowed by APs, and remove them
    swallowed = betwixt(t0+0.0001, [AP.t0].', [AP.t1].') & betwixt(t1-0.0001, [AP.t0].', [AP.t1].'); % each row represents single AP
