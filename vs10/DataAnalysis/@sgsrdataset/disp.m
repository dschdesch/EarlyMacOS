...
if prod(size(ds))>1,
   ...
   for ii=1:size(ds,1),
      for jj=1:size(ds,2),
         if isVec, coO = num2str(max(ii,jj));
         else, coO = [num2str(ii) ',' num2str(jj)];
         end
         iseq = getfield(ds(ii,jj), 'iseq');
         tit = getfield(ds(ii,jj), 'title');
...
IDstr = [IDstr ', ' Fm ' Seq=' num2str(getfield(ds, 'iSeq'))];
