...
IndepVal = unique(IndepVar.PlotVal(~isnan(IndepVar.PlotVal))); %For multiple independent variables ...
...
if ~isempty(IndepVar.ParUnit) && strcmp(StepMode, 'NONE')
    Txt = sprintf('[%.0f..%.0f] (%s)', min(IndepVal), max(IndepVal), IndepVar.ParUnit);
elseif strcmp(StepMode, 'NONE')
    Txt = sprintf('[%.0f..%.0f]', min(IndepVal), max(IndepVal));
elseif ~isempty(IndepVar.ParUnit) && strcmp(StepMode, 'SINGLETON'),
    Txt = sprintf('%.0f(%s)', IndepVal, IndepVar.ParUnit);
elseif strcmp(StepMode, 'SINGLETON')
    Txt = sprintf('%.0f', IndepVal);
elseif ~isempty(IndepVar.ParUnit) && strcmp(StepMode, 'LIN')
    Txt = sprintf('[%.0f..%.0f@%s] (%s)', min(IndepVal), max(IndepVal), Val2Str(Step), IndepVar.ParUnit);
elseif ~isempty(IndepVar.ParUnit)
    Txt = sprintf('[%.0f..%.0f@%s(Oct)] (%s)', min(IndepVal), max(IndepVal), Val2Str(Step), IndepVar.ParUnit);
