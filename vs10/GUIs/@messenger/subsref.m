...
% Messenger/SUBSREF - SUBSREF for Messenger objects
%
...
switch S.type,
    ...
    case '.',
        ...
        [Method, Mess] = keywordMatch(Field, methods(P), 'Messenger field name or method');
        if isempty(Mess), 
            Y = feval(Method, P);
        else, % try fieldname
            [Fldname, Mess] = keywordMatch(Field, fieldnames(P), 'Messenger field name or method');
            if isempty(Mess),
                Y = P.(Fldname);
            end
        end
        error(Mess);
    case '{}',
        error('Messenger objects do not allow subscripted reference using braces {}.');
end % switch/case
