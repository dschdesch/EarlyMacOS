...
% cycleList/SUBSREF - SUBSREF for cycleList objects
%
%   P.Foo is the the same as Foo(P) provided that Foo is a method for
%   cycleList objects.
%
%   See also cycleList, "methods cycleList".
...
switch S.type,
    ...
    case '.',
        ...
        [Method, Mess] = keywordMatch(Field, methods(P), 'cycleList field name or method');
        if isempty(Mess), 
            Y = feval(Method, P);
        else, % try fieldname
            [Fldname, Mess] = keywordMatch(Field, fieldnames(P), 'cycleList field name or method');
            if isempty(Mess),
                Y = P.(Fldname);
            end
        end
        error(Mess);
    case '{}',
        error('cycleList objects do not allow subscripted reference using braces {}.');
end % switch/case
