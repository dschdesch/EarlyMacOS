function P = stimparamsRF(S);
% stimparamsRF - generic stimulus parameters for RF stimulus protocol
%    G = stimparamsRF(S) takes the stimulus-defining struct S of an RF
...
%    GenericStimparams. The trinity of functions stimdefRF, stimparamsRF,
%    and makestimRF completely define the RF stimulus protocol. 
%   
%    StimparamsRF is called by stimGUI during the preparation of the 
%    auditory stimuli (see Stimcheck). For older datasets, which do
%    not store the generic parameters, StimparamsRF may also be called by
%    GenericStimparams during data analysis.
%
%    See also GenericStimparams, stimGUI, stimDefPath, makestimRF, 
%    stimparamsRF.
