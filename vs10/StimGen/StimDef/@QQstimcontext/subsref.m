...
%
%   P(5) is the 5th element of ActionButton array P.
...
switch S.type,
    ...
    case '.',
        ...
        [Method, Mess] = keywordMatch(Field, methods(P), 'StimulusContext field name or method');
        if isempty(Mess), 
            Y = feval(Method, P);
        else, % try fieldname
            [Fldname, Mess] = keywordMatch(Field, fieldnames(P), 'StimulusContext field name or method');
            if isempty(Mess),
                Y = P.(Fldname);
            end
        end
        error(Mess);
    case '{}',
        error('StimulusContext objects do not allow subscripted reference using braces {}.');
end % switch/case
