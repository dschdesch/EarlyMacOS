function S=Waveform(Fsam, DAchan, MaxMagSam, SPL, Param, Samples, Nrep, NeedsAnnotation);
...
elseif nargin<1, % void object with correct fields
    ...
    Annotations.Nrep = [];
    Annotations.Length = [];
    Annotations.BeginOfBaseline = 0;
    S = CollectInStruct(Fsam, DAchan, MaxMagSam, SPL, Param, Samples, Nrep, Nwav, NsamStore, NsamPlay, UniqueID,Annotations);
    S = class(S, mfilename);
    return;
elseif nargin<8,
    NeedsAnnotation = 0;
...
% Annotate these Samples as being a Stimulus
if nargin >= 8
    if NeedsAnnotation == 1
        for i=1:length(Samples)
           Annotations.Label{i} = 'stim';
           Annotations.Nrep(i) = Nrep(i);
           Annotations.Length(i) = length(Samples{i});
           Annotations.BeginOfBaseline = 0;
        end
    end
else
