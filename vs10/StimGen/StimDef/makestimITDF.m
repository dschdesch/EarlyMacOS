function P2=makestimITDF(P);
% MakestimITDF - stimulus generator for ITDF (ITD & freq steps) stimGUI
%    P=MakestimITDF(P), where P is returned by GUIval, generates the stimulus
%    specified in P. MakestimITDF is typically called by StimGuiAction when
%    the user pushes the Check, Play or PlayRec button.
%    MakestimITDF does the following:
...
%
%    MakestimITDF renders P ready for D/A conversion by adding the following 
%    fields to P
...
%             ITD: ITDs [ms] of all the presentations in an
%                  Nx2 matrix or column array
...
% ITD
ITD=EvalITDstepper(figh, P); 
if isempty(ITD), return; end
% split ITD in different types
[P.FineITD, P.GateITD, P.ModITD] = ITDparse(ITD, P.ITDtype);

% mix Freq & SPL sweeps; # conditions = # Freqs times # SPLs. By
% convention, freq is updated faster. 
[P.Fcar, P.ITD, P.Ncond_XY] = MixSweeps(Fcar, ITD);
...
if prod(P.Ncond_XY)>maxNcond,
    ...
    GUImessage(figh, Mess, 'error', {'StartFreq' 'StepFreq' 'EndFreq' 'startITD' 'stepITD' 'endITD' });
end
...
if ~okay, return; end
P.WavePhase = 0; P.WavePhaseUnit = 'cycle';
...
P = sortConditions(P, {'Fcar' 'ITD'}, {'Carrier frequency' 'ITD'}, ...
    {'Hz' 'ms'}, {P.StepFreqUnit 'Linear'});

% Levels and active channels (must be called *after* adding the baseline waveforms)
[mxSPL P.Attenuation] = maxSPL(P.Waveform, P.Experiment);
okay = CheckSPL(figh, P.SPL, mxSPL, P.Fcar, '', {'SPL'});
if ~okay, return; end
% Summary
