...
[Fcar, ModFreq, ModDepth, ModStartPhase, ModTheta, ...
    ...
    FineITD, GateITD, ModITD, SPL,StepFactor ,StepDelay] ...
    ...
    P.FineITD, P.GateITD, P.ModITD, P.SPL,P.StepFactor ,P.StepDelay);
...
[DAchanStr, Fcar, ModFreq, ModDepth, ModStartPhase, ModTheta, ...
    OnsetDelay, BurstDur, RiseDur, FallDur, ISI, WavePhase, ... 
    FineDelay, GateDelay, ModDelay, SPL, StepFactor, StepDelay] ...
...
SteadyDur = StepDelay-RiseDur; % steady (non-windowd) part of tone
[NonsetDelay, NgateDelay, Nrise, Nsteady, Nfall] = ...
    NsamplesofChain([OnsetDelay, GateDelay, RiseDur, SteadyDur, 0], Fsam/1e3);
Nfall_step = Nfall; % the step needs the fall duration
Nfall = 0;
...
wtone = ExactGate(wtone, Fsam, StoreDur-GateDelay, GateDelay, RiseDur, 0);
...
if useCyclicStorage, % cyclic storage
    ...
    NrepCyc_step = 1; % # reps of cyclic buffer
    NsamTail_step = 0; % No tail buffer
    Fcar = C.Fcar; Fmod = C.Fmod; % actual frequencies used in waveforms
end
%=======FREQUENCIES, PHASES, AMPLITUDES and CALIBRATION=======
isModulated = ~isequal(0,ModDepth) && ~isequal(0,Fmod);
...
dt = 1e3/Fsam; % sample period in ms
% compute dur of stored tone, whether useCyclicStorage or not
StoreDur = dt*(NgateDelay_step + Nrise_step + (NsamCyc_step + NsamTail_step) + Nfall_step); % only a single cycled buf is used
wtone_step = tonecomplex(Amp, freq, StartPhase, Fsam, StoreDur); % ungated waveform buffer; starting just after OnsetDelay
if logical(EXP.StoreComplexWaveforms); % if true, store complex analytic waveforms, real part otherwise
    wtone_step = wtone_step+ i*tonecomplex(Amp, freq, StartPhase+0.25, Fsam, StoreDur);
end
...
W = Waveform(Fsam, DAchan, NaN, SPL, Param, ...
    ...
    [NonsetDelay    1                  NrepCyc                       1 ...
     NonsetDelay_step    1                  NrepCyc_step                       1], 1);
