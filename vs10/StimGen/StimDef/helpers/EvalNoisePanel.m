...
if isfield(P,'ConstNoiseSeed')
    RandSeed = SetRandState(P.ConstNoiseSeed);
    P.RandSeed = RandSeed;
end
...
if isfield(P,'EndSPL')
    if mod(P.EndSPL - P.StartSPL,P.StepSPL) ~= 0
        GUImessage(figh, 'Stepsize does not exactly fit SPL bounds, Adjust SPL parameters', ...
            'error', FQnames);
        somethingwrong=1;   
    else
        P.SPL = [P.StartSPL:P.StepSPL:P.EndSPL]';
    end
end
