function C = measureTransfer(Fsam, Flow, Fhigh, Amp, FMspeed, ichan_out, ichan_in);
% measureTransfer - measure transfer function using RP2 DAC & ADC
%   C = measureTransfer(Fsam, Flow, Fhigh, Amp, FMspeed, ichan_out, ichan_in);
...
if nargin<2, Flow = []; end; % default value; see below
if nargin<3, Fhigh = []; end % default value; see below
if nargin<4, Amp = 0.1; end % Volt default stim ampl
if nargin<5, FMspeed=1; end % default 1 oct/s
if nargin<6, ichan_out=1; end
if nargin<7, ichan_in=1; end

if isempty(Flow), Flow = 100; end % Hz 
if isempty(Fhigh), Fhigh = 1e3*Fsam*0.45; end % practical Nyquist [Hz]
...
% f1; dplot(S.dt, S.N); xdplot(S.dt, Rec,'r');
[XX, iilag] = xcorr(Rec, S.N);
% f2; plot(S.dt*iilag, XX); 
[MX, imax] = max(XX);
NsamLag = iilag(imax);
Lag_ms = NsamLag*S.dt;
...
C = CollectInStruct(Fsam, Amp, FMspeed, ichan_out, ichan_in, NsamOctWin, EdgeFreq, NsamLag, Lag_ms, Freq, TRF);
