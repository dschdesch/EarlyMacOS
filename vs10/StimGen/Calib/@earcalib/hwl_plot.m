function ah=hwl_plot(EC, varargin);
...
%    plot(ah(1:2), mFactor*Trf1, lico(icol1), 'linewidth', 1.5, varargin{:});
    plot(mFactor*Trf1, 'linewidth', 1.5);
%    axes(xlog125(fmin(Trf1)/1e3, fmax(Trf1)/1e3));
%    set('xlim', [0, fmax(Trf1)]/1e3);
%    ylabel('Max Tone Level (dB SPL)');
...
%    plot(ah(3:4), mFactor*Trf2, lico(icol2), 'linewidth', 1.5, varargin{:});
    plot(mFactor*Trf2, 'linewidth', 1.5);
%    axes(xlog125(fmin(Trf2)/1e3, fmax(Trf2)/1e3));
%    set('xlim', [0, fmax(Trf2)]/1e3);
%    ylabel('Max Tone Level (dB SPL)');
