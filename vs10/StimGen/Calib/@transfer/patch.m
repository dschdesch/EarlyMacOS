...
%	EVE: 07-02-2017 formula to stich was opposite and now smoother transient
...
if NsamBoth>0,
    ...
    w = 1 - cos(w*pi/2).^2;  %EVE 20/12/2015 smoother transient
%     P.Ztrf(inBoth) = w.*Sz(inBoth) + (1-w).*Tz(inBoth); % EVE 20/12/2015 error: fade S out, fade T in
    P.Ztrf(inBoth) = w.*Tz(inBoth) + (1-w).*Sz(inBoth); % fade S out, fade T in    
end
